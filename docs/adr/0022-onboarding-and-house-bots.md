...
3. `git clone <quickstart-repo>` → `pip install -r requirements.txt` (installs the `engineroom` SDK, ADR-0021).
4. Paste the key into `.env` (`ENGINEROOM_KEY=...`; legacy `CHESSROOM_KEY` still accepted, deprecated — KAN-71) → `python random_bot.py`.
...
The onboarding path (steps 3–6) is real. The **quickstart template** (`sdk/quickstart`: `random_bot.py` + `.env.example` + README + optional Dockerfile) delivers `git clone → uv sync → paste ENGINEROOM_KEY into .env → uv run python random_bot.py → matched vs the house → watch on the V6 dashboard`; an **SDK-fed Playwright e2e** proves it end-to-end (the ADR-0023 smoke). The quickstart **depends on** the packaged `engineroom` SDK (path-installed pre-PyPI, ADR-0021 V7 addendum), not a vendored copy — as this ADR intended.
...
## Addendum (2026-07-10, post-MVP): two house-bot personas — ephraim (greeter) + jian (ambient)
The two house *roles* now have two distinct *identities and strengths* (was: both shared the single `house-random` identity):

- **`ephraim-bot`** — the **Kind-2 ephemeral greeter**. A deliberately **easy, `RandomBot`** first opponent for a newcomer's lone seek; plays one game and is gone. Given its own seeded identity (`bot_ephraim`) so it's no longer conflated with a permanent bot. Its games are still rated (single persistent row), but its rating **drift is ignored** — it isn't a lobby resident (satisfies "rating doesn't matter" without touching the finalize path).
- **`jian-bot-001` / `jian-bot-002`** — the **Kind-1 permanent** ambient residents. They now play **minimax + alpha-beta** (`game.minimax`, depth `ER_AMBIENT_MINIMAX_DEPTH`, default 3) so lobby games look like real chess, and remain **rated + persisted** and free to play each other (created outside the matcher). The two `house-random`/`house-random-2` rows keep their **IDs** (FK/history preserved); Alembic **0005** renames them and seeds `ephraim-bot`.

Still **single-process / in-memory** (ADR-0020): the ambient bots run as in-process movers via `AmbientSupervisor`. Running house bots as **out-of-process SDK WebSocket clients** (the clean scale-up path, now that V7's SDK exists — it would also fold them into the real matcher with a house same-owner exemption) is **deferred** to a later change. See CLAUDE.md build-status ("house-bot personas") and `game/house_bots.py`.

## Addendum (2026-07-14, post-MVP): spectator-gate the ambient feeder (KAN-209)
The *never-empty-lobby* guarantee only matters **while someone is watching** — the ambient feeder previously ran the two `jian-bot` minimax bots 24/7, burning Neon-row growth + compute against an empty lobby. KAN-209 makes it spectator-gated:

- A tiny in-memory **`Presence`** last-seen signal (`spectate/presence.py`, injectable clock for deterministic tests) is **bumped** by any `GET /api/games` lobby poll **or** a watch SSE connect. **Online = lobby OR watch activity in the last `ER_AMBIENT_PRESENCE_WINDOW_SECONDS`** (default 60s). Counting lobby polls is deliberate: if no house game is running there's nothing to spectate, so lobby *browsing itself* must trigger games — otherwise a drained lobby would never refill.
- The `AmbientSupervisor` reads `presence.is_fresh()`: it **only launches new games while presence is fresh**, and once it goes **stale it stops refilling** — in-flight games **finish naturally (never aborted)**, so the pool drains to 0. A background **poll loop** (`ER_AMBIENT_PRESENCE_POLL_SECONDS`, default 5s) **cold-starts** the lobby when a watcher returns (a game finishing is the only other refill trigger, and there are none once drained → the minor cold-start latency the ticket notes as pre-warmable).
- **Cold-start is staggered** (`ER_AMBIENT_SPAWN_STAGGER_SECONDS`, default 2s). Spawning all `ambient_games` at once fires that many **simultaneous opening minimax searches** — a CPU/GIL spike (searches run in `to_thread`, but pure-Python minimax holds the GIL) that, because presence now goes fresh *at dashboard load*, coincides with the test/user critical window and can starve the loop (it broke the `sdk.spec` e2e — the concurrent SDK greeter game's card didn't render in 45s). A gap between launches phase-shifts the searches, exactly as steady-state one-at-a-time respawns already do; a normal single-game respawn on finish never sleeps. Before KAN-209 this burst happened harmlessly at server startup, long before any test navigated.
- **Supersedes** the blunt `ER_AMBIENT_GAMES=0` manual pause (that env knob still works). With KAN-203's off-the-event-loop move search, depth 3 is safe again, so the prod `ER_AMBIENT_MINIMAX_DEPTH=2` / `ER_AMBIENT_GAMES=0` overrides can be relaxed (ops action — not a repo default change; the repo already defaults to 2 games / depth 3).
- Injected only in `create_app` (production wiring); the DB-free unit and Postgres integration tests that drive `AmbientSupervisor` directly pass **no** presence signal → **always-on**, preserving pre-KAN-209 behaviour.
