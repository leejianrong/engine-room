...
- **e2e knock-on (`sdk.spec.ts`):** gating changes *registry order* at test time — the SDK/greeter game is now the first live game (ambient is dormant until the dashboard's first lobby poll makes presence fresh), so it wins the dashboard's "Featured game" slot (all games tie at 0 watchers / 2400 combined rating → first-in-registry wins), which the "Live games" list explicitly excludes. The old test locator was scoped to the "Live games" section, so it stopped matching. Fixed by broadening the locator to assert the SDK bot appears **anywhere** on the dashboard (featured or the list) and is watchable — not a server-behaviour change.
